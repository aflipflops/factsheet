...
### Objective
The NDC2018 realisation will succeed the old NDC realisation in the module _40_techpol_ in REMIND. The aim of this file is to compare results between the old and new realisation.

#### Detail
The new realisation contains capacity, share, and emission targets from NDCs that are generated from `moinput` and are compatible with the flexible regions in REMIND. Some policy targets have been hard-coded, when necessary.

### Results
The first implementation contains only capacity targets and these have been compared here. The targets are created and stored in `f40_NDC.cs4r` (in the input folder of the module 40_techpol) and serve as lower bounds for the "capacity" variables.
