...
ggplot()+
  ...
  line_type(whole_a) +
  annotate(geom = "point",x = c(2005,2010),y=filtered$Value)
...
whole_c <- whole_c %>% 
  mutate(share=value/whole_a$value) %>% 
  filter(period<=2050) 
...
```{r}
  ggplot()+
  geom_line(data = whole_c %>% filter(model %in% c("India MARKAL","AIM/Enduse 3.0")), aes(x=period,y=share,group=interaction(model,scen_name),linetype=scen_name,size=model)) +
geom_line(data = whole_c %>% filter(!model %in% c("India MARKAL","AIM/Enduse 3.0")), aes(x=period,y=share,group=interaction(model,scen_name),color=model,linetype=scen_name)) + 
  labs(
    x = "Year" ,
    y = "Share of Electricity to total CO2 Emissions",
    color = "Global model",
    linetype = "Scenario Type",
    size="National model"
  ) +  theme_bw()+ 
  scale_colour_brewer(palette = "Set1")+
  coord_cartesian(xlim = c(2005,2050),ylim=c(-0.4,1))+
  scale_size_discrete(range = c(0.95, 0.65))
ggsave(file="CO2emi_elec_share_IND.png",width=8,height=4)

ggplot()+
  geom_bar(data=whole_c %>% filter(period==2010),aes(x=reorder(model,share),y=share, group=interaction(model,scen_name),fill=scen_name,alpha=model_scope),position="dodge", stat = "identity")+coord_flip()+
  scale_alpha_discrete(range = c(0.95, 0.65))+
  labs(
    x = "Year" ,
    y = "CO2 Emissions Electricity (Gt/yr)")
...
ggplot()+
  geom_bar(data = whole_c %>% filter(model %in% c("India MARKAL","AIM/Enduse 3.0")), aes(x=period,y=share,group=interaction(model,scen_name),fill=model_scope),stat = "identity") +
geom_bar(data = whole_c %>% filter(!model %in% c("India MARKAL","AIM/Enduse 3.0")), aes(x=period,y=share,group=interaction(model,scen_name),fill=model_scope),stat = "identity")+
  labs(
    x = "Year" ,
    y = "CO2 Emissions Electricity (Gt/yr)",
...
ggplot()+
  geom_line(data = whole_d , aes(x=period,y=value,group=interaction(model,scen_name),linetype=scen_name,color=model_scope)) +
   labs(
    x = "Year" ,
...
ggplot()+
  geom_line(data = whole_d %>% filter(model %in% c("India MARKAL","AIM/Enduse 3.0")), aes(x=period,y=value,group=interaction(model,scen_name),linetype=scen_name,size=model)) +
geom_line(data = whole_d %>% filter(!model %in% c("India MARKAL","AIM/Enduse 3.0")), aes(x=period,y=value,group=interaction(model,scen_name),color=model,linetype=scen_name)) + 
  labs(
    x = "Year" ,
    y = "Final Energy Electricity (EJ/yr)",
    color = "Global model",
...
whole_e <- whole[whole$variable==plotvar,]
localrate = 0.015 #https://www.xe.com/currencycharts/?from=INR&to=USD&view=10Y
whole_e[whole_e$model %in% c("India MARKAL","AIM/Enduse 3.0"),]$value <- whole_e[whole_e$model %in% c("India MARKAL","AIM/Enduse 3.0"),]$value*localrate
filtered <- hist_and_botmup %>% filter(Region=="IND", Year %in% seq(2015,2040,5),Variable =="spv",Source=="IEA_WEO") 

ggplot()+
  geom_line(data = whole_e,aes(x=period,y=value,group=interaction(model,scen_name),color=model,size=model_scope)) +
  labs(
    x = "Year" ,
    y = "Tech Cost PV ($/W)",
    color = "Model name",
  ...
  scale_size_discrete(range = c(0.65, 0.95))+
  coord_cartesian(xlim = c(2005,2050)) +
  annotate(geom = "point",x = seq(2015,2040,5),y=filtered$Value)
  # scale_colour_brewer(palette = "Set1")+
  theme_light()
...
whole_f <- whole[whole$variable==plotvar,]    
filtered <- hist_and_botmup %>% filter(Region=="IND", Year %in% c(2005,2010), Variable =="SE|Electricity|Coal (EJ/yr)")

ggplot()+
  geom_line(data = whole_f , aes(x=period,y=value,group=interaction(model,scen_name),linetype=scen_name,color=model_scope)) +
   labs(
    x = "Year" ,
    y = "Secondary Energy Electricity (EJ/yr)",
    color = "Model Scope",
    linetype = "Scenario Type")+
  theme_bw()+ 
  coord_cartesian(xlim = c(2005,2050))+ 
...
ggplot()+
  geom_line(data = whole_f %>% filter(model %in% c("India MARKAL","AIM/Enduse 3.0")), aes(x=period,y=value,group=interaction(model,scen_name),linetype=scen_name,alpha=model)) +
geom_line(data = whole_f %>% filter(!model %in% c("India MARKAL","AIM/Enduse 3.0")), aes(x=period,y=value,group=interaction(model,scen_name),color=model,linetype=scen_name)) + 
  labs(
    x = "Year" ,
    y = "Seconda Energy Electricity (EJ/yr)",
    color = "Global model",
    linetype = "Scenario Type",
    alpha="National model"
  ) +  theme_bw()+ 
...
whole_g <- whole_g[whole_g$variable==plotvar,]    
filtered <- hist_and_botmup%>%
  filter(grepl("Operating|Constr|Announced",Variable),Region=="IND")
ggplot()+
  geom_line(data = whole_g,aes(x=period,y=value,group=interaction(model,scen_name),color=model)) +
  labs(
    x = "Year" ,
