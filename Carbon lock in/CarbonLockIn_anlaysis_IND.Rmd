...
```{r all_data,include=FALSE,eval=FALSE,message=FALSE,purl=FALSE,warning=FALSE}
#read in data
...
whole <- whole %>% 
  filter(!model %in% c("COPPE-COFFEE 1.0","AIM/CGE"))
```
...
myColors <- brewer.pal(11,"Paired") # for unique colors on models
...
line_type_2 <- function(data_s,hist_s){
  ggplot()+
  geom_line(data = data_s %>% filter(model %in% c("India MARKAL","AIM/Enduse 3.0")), aes(x=period,y=value,group=interaction(model,scen_name),linetype=scen_name,alpha=model),size=1) +
geom_line(data = data_s %>% filter(!model %in% c("India MARKAL","AIM/Enduse 3.0")), aes(x=period,y=value,group=interaction(model,scen_name),color=model,linetype=scen_name))+
  ...
  scale_alpha_discrete(range = c(0.90, 0.60))+
    geom_line(data = hist_s%>%mutate(dummy="Historical"),aes(x = as.numeric(as.character(Year)),y=Value,fill=dummy),size=1.5,shape=17)
...
line_type_2(whole_emi,hist_emi)+
  geom_ribbon(data = whole_emi_m %>% filter(model_scope=="global",period>=2015),aes(x =period,ymin = minimum,ymax=maximum),alpha = 0.1,fill="blue")+
geom_ribbon(data = whole_emi_m %>% filter(model_scope=="national",period>=2015),aes(x = period,ymin = minimum,ymax=maximum),alpha = 0.1,fill="orange")
...
line_type_2(whole_emi_elec %>% filter(scen_name %in% c("Delayed action")),hist_emi_elec)+
  coord_cartesian(ylim=c(0,2500),xlim=c(2005,2050))+ labs(y="CO2 Emissions from Electricity  (MtCO2/yr)")+theme(axis.text=element_text(size=12), axis.title=element_text(size=13,face="bold"),legend.position="none")


# +
#  geom_ribbon(data = whole_emi_elec_m %>% filter(model_scope=="global",period>=2015),aes(x =period,ymin = minimum,ymax=maximum),alpha = 0.1,fill="blue")+
# geom_ribbon(data = whole_emi_elec_m %>% filter(model_scope=="national",period>=2015),aes(x = period,ymin = minimum,ymax=maximum),alpha = 0.1,fill="orange")

# WTHOUT RIBBONS
line_type_2(whole_emi_elec,hist_emi_elec)+theme(axis.text=element_text(size=12), axis.title=element_text(size=13,face="bold"))
...
line_type_2(whole_emi_elec_share,hist_emi_elec_share)+
  ...
  geom_ribbon(data = whole_emi_elec_share_m %>% filter(model_scope=="global",period>=2015),aes(x =period,ymin = minimum,ymax=maximum),alpha = 0.1,fill="blue")+
geom_ribbon(data = whole_emi_elec_share_m %>% filter(model_scope=="national",period>=2015),aes(x = period,ymin = minimum,ymax=maximum),alpha = 0.1,fill="orange")+theme(legend.text=element_text(size=10))
...
### DATA 
whole_emi_elec <- whole[whole$variable=="Emissions|CO2|Energy|Supply|Electricity",]
whole_emi_elec <- whole_emi_elec %>% filter(scen_name=="Early action")

try_2 <- transform(unique(whole_emi_elec$modscen),cumsum=whole_emi_elec[which(whole_emi_elec$period==2010),]$value*2.5 + whole_emi_elec[which(whole_emi_elec$period==2015),]$value*5 + 
           whole_emi_elec[which(whole_emi_elec$period==2020),]$value*5 + whole_emi_elec[which(whole_emi_elec$period==2025),]$value*5 +
           whole_emi_elec[which(whole_emi_elec$period==2030),]$value*2.5
           )
whole_emi_elec <- whole[whole$variable=="Emissions|CO2|Energy|Supply|Electricity",]
whole_emi_elec <- whole_emi_elec %>% filter(scen_name=="Delayed action")

try_3 <- transform(unique(whole_emi_elec$modscen),cumsum=whole_emi_elec[which(whole_emi_elec$period==2010),]$value*2.5 + whole_emi_elec[which(whole_emi_elec$period==2015),]$value*5 + 
           whole_emi_elec[which(whole_emi_elec$period==2020),]$value*5 + whole_emi_elec[which(whole_emi_elec$period==2025),]$value*5 +
           whole_emi_elec[which(whole_emi_elec$period==2030),]$value*2.5)

           
names(try_2)[1] <- "modscen"
names(try_3)[1] <- "modscen"

try <- rbind(try_2,try_3)
whole_cum <- left_join( whole[whole$variable=="Emissions|CO2|Energy|Supply|Electricity",],try,by="modscen")


## Making table
whole_cum_table <- whole_cum %>% 
  filter(period==2050) %>% 
  arrange(scen_name) %>% 
  select(model,model_scope,scen_name,cumsum) %>% 
  spread(c("scen_name"),cumsum)



ggplot()+
   geom_bar(data=whole_emi_elec %>% filter(period==2030)  , aes(x=model,y=value/1000, fill=scen_name),position="dodge", stat = "identity")+
   labs(
     x = "Model name" ,
     y = "Cum. Emissions/Power sector (2011-2030) (GtCO2)",
     fill = "Scenario Type",
     alpha = "Model Scope"
   ) +
    theme_bw()+
...
whole_cum_energy <- whole[whole$variable %in% c("Emissions|CO2|Energy|Supply|Electricity","Emissions|CO2|Energy|Demand|Industry","Emissions|CO2|Energy|Demand|Residential and Commercial","Emissions|CO2|Energy|Demand|Transportation"), ]

# creating empty data frame
try_3 <- data.frame(modscen=factor(),
                 cumsum=double(),
                 variable=factor(),
                 scen_name=factor(),
                 stringsAsFactors=FALSE)
# storing cumulative  emissions from 2015 to 2030 into cumsum
for (j in unique(whole_cum_energy$scen_name)){
 for (i in unique(whole_cum_energy$variable)){
   whole_cum_energy_2 <- whole_cum_energy %>% 
     filter(scen_name==j & variable==i)
    # try_2 <- transform(unique(whole_cum_energy_2$modscen),cumsum= whole_cum_energy_2[which(whole_cum_energy_2$period==2015),]$value*2.5 + 
    #        whole_cum_energy_2[which(whole_cum_energy_2$period==2020),]$value*5 + whole_cum_energy_2[which(whole_cum_energy_2$period==2025),]$value*5 +
    #        whole_cum_energy_2[which(whole_cum_energy_2$period==2030),]$value*2.5)
         try_2 <- transform(unique(whole_cum_energy_2$modscen),cumsum=            whole_cum_energy_2[which(whole_cum_energy_2$period==2030),]$value*1)
   names(try_2)[1] <- "modscen"
   try_2$variable <- i
   try_2$scen_name <- j
   try_3 <- rbind(try_3,try_2)
 }
}

# adding cumsum to original dataframe
whole_cum <- left_join( whole_cum_energy,try_3,by=c("modscen","variable","scen_name"))

# whole_cum_2 <- transform(whole_cum,excess=whole_cum[whole_cum$scen_name=="Delayed action",]$cumsum-whole_cum[whole_cum$scen_name=="Early action",]$cumsum)

# spreading data
whole_cum_3 <- spread(data = whole_cum,key = scen_name, value = cumsum)

# selected only delayed emissions and removing NA
whole_cum_4 <- whole_cum_3 %>% 
  select(1:10)
whole_cum_4 <- drop_na(whole_cum_4,10)

whole_cum_5 <- whole_cum_3 %>% 
  select(c(1:9,11))
whole_cum_5 <- drop_na(whole_cum_5,10)

whole_cum_6 <- cbind(whole_cum_4,whole_cum_5[10])
whole_cum_7 <- whole_cum_6 %>% 
  mutate(Excess= `Delayed action`- `Early action`)

  
whole_cum_8 <- gather(whole_cum_7, scen_name=c('Excess','Early action','Delayed action'),key = scen_name,value = value)
whole_cum_8 <- whole_cum_8 %>% filter(period==2005)
#whole_cum_8$variable <- mip::shorten_legend(whole_cum_8$variable,35)

# source("plotstyle.R")

  ggplot()+
   geom_bar(data=whole_cum_8  , aes(x=scen_name,y=value/1000, fill=variable),position="stack", stat = "identity")+
   labs(
     x = "Model name" ,
     y = "Cum. Emissions (2016-2030) (GtCO2)",
     fill = "Scenario Type",
     alpha = ""
   ) +
    theme_bw()+
     scale_alpha_discrete(range = c(0.95, 0.55)) +
     scale_fill_manual(values=plotstyle(unique(whole_cum_8$variable)))+
  facet_grid(~model_scope+model,scales="free",space = "free_x",labeller = label_wrap_gen(width =10, multi_line = TRUE))+
    theme(axis.text.x = element_text(angle = 90, hjust = 1),legend.direction = "vertical",legend.position = "bottom" )
  
    ggplot()+
   geom_bar(data=  whole_cum_energy %>% filter(period==2030)  , aes(x=scen_name,y=value/1000, fill=variable),position="stack", stat = "identity")+
   labs(
     x = "Model name" ,
     y = "Cum. Emissions (2016-2030) (GtCO2)",
     fill = "Scenario Type",
...
# whole_cum_energy <- whole_cum_energy %>% 
#   filter(scen_name=="Early action") %>% 
#   group_by(modscen,variable) %>% 
  # mutate(cumsum=whole_cum_energy[which(whole_cum_energy$period==2015),]$value*2.5 + 
  #          whole_cum_energy[which(whole_cum_energy$period==2020),]$value*5 + whole_cum_energy[which(whole_cum_energy$period==2025),]$value*5 +
  #          whole_cum_energy[which(whole_cum_energy$period==2030),]$value*2.5)
...
ggsave("high_carbon.svg",device = "svg",height=5,width = 6)
```
...
line_type_2(whole_se_elec_coal,hist_se_elec_coal)

ggplot()+
  geom_line(data = whole_se_elec_coal , aes(x=period,y=value,group=interaction(model,scen_name),linetype=model_scope,color=model))+facet_grid(~scen_name)+
 labs(
    x = "Year",
...
# SE|Gas|Electricity
whole_se_elec_gas <- whole %>% filter(variable %in% c("Secondary Energy|Electricity|Gas|w CCS","Secondary Energy|Electricity|Gas|w/o CCS"))%>% 
 group_by(model, scenario,unit, period,modscen, model_scope, scen_name,region,variable) %>% 
  summarise(value=sum(value))

hist_se_elec_gas <- hist_and_botmup %>% filter(Region=="IND", Year %in% c(2005:2015), Variable=="SE|Electricity|Gas (EJ/yr)")

ggplot()+
  geom_line(data =whole_se_elec_gas %>% filter(period<2050) , aes(x=period,y=value,group=interaction(model,scen_name),linetype=model_scope,color=model))+facet_grid(~scen_name)+
 labs(
    x = "Year",
    y = "Secondary Energy|Electricity|Gas(EJ/yr)" )+ theme_bw()+
   annotate("point",x=c(2005:2015),y=hist_se_elec_gas$Value)
 
line_type_2(whole_se_elec_gas %>% filter(scen_name=="Delayed action"),hist_se_elec_gas )+ labs(
    x = "Year",
    y = "Secondary Energy|Electricity|Gas(EJ/yr)" )+theme(axis.text=element_text(size=10), axis.title=element_text(size=14,face="bold"))
...
line_type_2(whole_fe_elec,hist_fe_elec)+theme(axis.text=element_text(size=14), axis.title=element_text(size=18,face="bold"))
...
### Capacity factor for India AIM/ENDUSE and Capacity for Delayed Section

whole_se_elec_coal <- whole %>% filter(variable %in% c("Secondary Energy|Electricity|Coal|w/o CCS",
                                                       "Secondary Energy|Electricity|Coal|w/ CCS"))

whole_se_elec_coal_2 <- whole_se_elec_coal %>%
  filter(model %in% c("AIM/Enduse 3.0"),variable=="Secondary Energy|Electricity|Coal|w/o CCS",scen_name=="Early action") 


whole_coal_cap <- whole %>% filter(variable %in% c("Capacity|Electricity|Coal|w/o CCS")) 
whole_coal_cap_2 <- whole_coal_cap %>% 
  filter(model %in% c("AIM/Enduse 3.0"))


whole_cap_fac <- whole_coal_cap_2 %>% 
  group_by(modscen) %>% 
  mutate(cap_factor = whole_se_elec_coal_2$value*277777.778/(value*8760))

whole_se_elec_coal_3 <- whole_se_elec_coal %>% 
  filter(model %in% c("AIM/Enduse 3.0"),scen_name=="Delayed action",variable=="Secondary Energy|Electricity|Coal|w/o CCS") 
whole_se_elec_coal_3 <- whole_se_elec_coal_3 %>% 
  mutate(capacity=whole_se_elec_coal_3$value*277777.778/(whole_cap_fac$cap_factor*8760))%>% 
  select(model, scenario,unit, period,modscen, variable, model_scope, scen_name,region,capacity) %>% 
  rename_(value=10)

whole_se_elec_coal_3$variable <- "Capacity|Electricity|Coal|w/o CCS"
...
whole_cap <- rbind(whole_cap,whole_se_elec_coal_3)
...
#### Coal Capacity
```{r coal_plant_eff,warning=FALSE,fig.show='hold',out.width="50%",warning=FALSE}

whole_coal_cap <- whole %>% filter(variable %in% c("Capacity|Electricity|Coal|w/o CCS"))

# since whole_coal_cap doesn't contain values for AIM/ENDUSE Delayed Scenario, 
whole_coal_cap_2 <- rbind(whole_coal_cap,whole_se_elec_coal_3) 
...
line_type_2(whole_coal_cap_2,hist_coal_cap[hist_coal_cap$Variable=="Operating",])+   geom_line(data = hist_2,aes(y = value/1000,x=Year),color="black",size=1.5)+
      geom_hline(yintercept=hist_coal_cap[hist_coal_cap$Variable=="Operating",]$Value)+
        geom_text(x=2015,y=230,aes(label="Operating"),check_overlap = T)+
          geom_hline(yintercept =  hist_coal_cap[hist_coal_cap$Variable=="Operating",]$Value+hist_coal_cap[hist_coal_cap$Variable=="Construction",]$Value)+            geom_text(x=2015,y=270,aes(label="Under Construction"),check_overlap = T)+
              geom_hline(yintercept = hist_coal_cap[hist_coal_cap$Variable=="Operating",]$Value+hist_coal_cap[hist_coal_cap$Variable=="Announced + Pre-permit + Permitted",]$Value+hist_coal_cap[hist_coal_cap$Variable=="Construction",]$Value)+
  geom_text(x=2020,y=340,aes(label="Under construction + planned"),check_overlap = T)+
...
whole_se_elec_coal_2 <- whole_se_elec_coal %>% filter(variable!="Secondary Energy|Electricity|Coal|w/ CCS") %>%  arrange(modscen)

whole_coal_cap <- whole %>% filter(variable %in% c("Capacity|Electricity|Coal|w/o CCS"))

# since whole_coal_cap doesn't contain values for AIM/ENDUSE Delayed Scenario, 
whole_coal_cap <- rbind(whole_coal_cap,whole_se_elec_coal_3) 

whole_cap_factor <- whole_coal_cap %>%   arrange(modscen)
...
ggplot()+
  geom_line(data = whole_cap_factor, aes(x=period,y=cap_factor,group=interaction(model,scen_name),linetype=model_scope,color=model)) +
   labs(
    x = "Year" ,
    y = "Capacity Factor",
    color = "Model Scope",
