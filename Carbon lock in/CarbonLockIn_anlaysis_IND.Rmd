...
########## PLOT
line_type_2(whole_emi_elec_share,hist_emi_elec_share)+coord_cartesian(ylim=c(0.75,0))
```
...
#
whole_try <- whole_se_elec_coal %>%
  filter(!modscen %in% c("COPPE-COFFEE 1.0 - NPi2020_1000_V3","COPPE-COFFEE 1.0 - INDC2030i_1000_V3"))
whole_coal_cap <- whole_coal_cap

 whole_cap_factor <- whole_se_elec_coal %>%
  filter(!modscen %in% c("COPPE-COFFEE 1.0 - NPi2020_1000_V3","COPPE-COFFEE 1.0 - INDC2030i_1000_V3")) %>%
...
ggplot()+
  ...
  facet_grid(scen_name~model)
ggplot()+
  geom_bar(data=whole_cap_elec , aes(x=period,y=value,group=variable,fill=variable),position = "stack",stat = "identity")+facet_grid(period~model)
