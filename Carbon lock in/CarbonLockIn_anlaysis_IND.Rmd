...
```{r all_data,include=FALSE,results='hide',message=FALSE,purl=FALSE}
...
```{r historical data,include=FALSE,eval=F}
...
ggplot()+
  geom_line(data = whole_pe_fossil %>% filter(period<2050,scen_name=="Early action") , aes(x=period,y=value,group=interaction(model,scen_name),linetype=model_scope,color=model))+facet_grid(~variable)+
 labs(
    x = "Year" ,
...
whole_se_elec <-whole %>% filter(variable %in% grep("Secondary Energy|Electricity|",unique(whole$variable),value=T,fixed = T))
## adding all SE techs
...
ggplot()+
  geom_line(data = whole_se_coal_2 , aes(x=period,y=cap_factor,group=interaction(model,scen_name),linetype=scen_name,color=model_scope)) +
   labs(
    x = "Year" ,
    y = "Capacity Factor",
    color = "Model Scope",
...
```{r req packages,warning=FALSE,include=FALSE}

# library(quitte)
# library(moinput)
list.of.packages <- c("tidyverse", "reshape","data.table")
...
```{r all_data,eval=FALSE,include=FALSE,message=FALSE,purl=FALSE}
...
```{r historical data,eval=FALSE, include=FALSE,echo=FALSE,,message=FALSE,purl=FALSE}
# Relevant historical and bottom-up data 
# historical IEA data (PE, SE, FE,Trade), and CEDS emissions data
setConfig(forcecache = TRUE)
historical <- calcOutput("Historical",aggregate = F)# aggregate = T not working at the moment, use when available
...
##### CO2 Emissions
```{r co2 emissions,,warning=FALSE}

plotvar <- "Emissions|CO2"
whole_emi <- whole[whole$variable==plotvar,]
...
```{r cum_emi,message=FALSE,warning=FALSE,error=FALSE,purl=FALSE}
whole_cum <- whole_emi %>% 
  filter(period>2010 & period<2055) %>% 
  group_by(model,scen_name) %>% 
  mutate(cumsum=cumsum(value)/1000) %>% 
...
#### Emissions from the Electricity sector
```{r elec emissions,message=FALSE,warning=FALSE,error=FALSE,purl=FALSE}
#plot emissions electricity sector 
plotvar <- "Emissions|CO2|Energy|Supply|Electricity"
whole_emi_elec <- whole[whole$variable==plotvar,]
# historical emissions //India// Fossil Fuel and Industry
hist_emi_elec <- hist_and_botmup %>% filter(Region=="IND", Year %in% c(2005,2010),Variable =="Emi|CO2|Energy|Supply|Electricity (Mt CO2/yr)") 
# share is the share of electricity emissions to total emissions
line_type_1(whole_emi_elec,hist_emi_elec,c(2005,2010))
...
#### Final Energy of Electricity
```{r FE|Electricity,message=FALSE,warning=FALSE,error=FALSE,purl=FALSE}
plotvar <- "Final Energy|Electricity"
whole_fe_elec <- whole[whole$variable==plotvar,]
hist_fe_elec <- hist_and_botmup %>% filter(Region=="IND", Year %in% c(2005,2010),Variable =="FE|Electricity (EJ/yr)") 
...
#### Secondary Energy of Electricity
```{r SE|Electricity,message=FALSE,warning=FALSE,error=FALSE,purl=FALSE}
# all tech with secondary energy of electricity
plotvar <- grep("Secondary Energy|Electricity|",unique(whole$variable),value=T,fixed = T)
whole_se_elec <-whole[whole$variable %in% plotvar,]
## adding all SE techs
...
#### Secondary Energy from Coal 
```{r SE|Elec|Coal,include=FALSE}
plotvar <- "Secondary Energy|Electricity|Coal|w/o CCS"
whole_se_coal <- whole[whole$variable==plotvar,]    
hist_se_coal <- hist_and_botmup %>% filter(Region=="IND", Year %in% c(2005,2010), Variable =="SE|Electricity|Coal (EJ/yr)")
...
#### Coal Power Capacity
```{r Capacity|Elec|Coal,message=FALSE,warning=FALSE,error=FALSE,purl=FALSE}
plotvar <- "Capacity|Electricity|Coal|w/o CCS"
whole_coal_cap <- whole[whole$variable==plotvar,]    
# hist_coal_cap <- hist_and_botmup%>%
#   filter(grepl("Operating|Constr|Announced",Variable),Region=="IND")
...
whole_se_elec <-whole[whole$variable %in% plotvar & whole$period==2030,]
whole_se_elec_tot <- whole[whole$variable %in% "Secondary Energy|Electricity" & whole$period==2030,]
source("plotstyle.R")
...
>>>>>>> a906bc45c50cd1a124c98ffea9327e884bd859bb
