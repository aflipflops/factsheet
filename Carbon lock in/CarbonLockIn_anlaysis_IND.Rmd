...
# try 1
ggplot()+
  geom_bar(data=whole_b %>% filter(period==2050) , aes(x=reorder(model,cumsum),y=cumsum, group=interaction(model,scen_name),fill=scen_name),position="dodge", stat = "identity")+
  labs(
    x = "Model name" ,
...
geom_line(data = data_s %>% filter(!model %in% c("India MARKAL","AIM/Enduse 3.0")), aes(x=period,y=value,group=interaction(model,scen_name),color=model,linetype=scen_name))+
  ...
  annotate(geom = "point",x = years,y=hist_s$Value)
  ggsave(file=paste(gsub(c("\\||\\/"),"_",unique(data_s$variable)),"_2",".png",sep=""),width=8,height=4)
  }
...
whole_cap_cost[cap_cost$model %in% c("India MARKAL","AIM/Enduse 3.0"),]$value <- whole_cap_cost[whole_cap_cost$model %in% c("India MARKAL","AIM/Enduse 3.0"),]$value*localrate
...
#   filter(grepl("Operating|Constr|Announced",Variable),Region=="IND")
  ...
  line_type_1(whole_coal_cap,hist_coal_cap,2020)
  line_type_2(whole_coal_cap,hist_coal_cap,2020)
```
...
## what <- grep("Coal",unique(whole$variable),value=T)
...
levels(whole_se_elec$model) <- 
source("plotstyle.R")

ggplot()+
  geom_col(data=whole_se_elec , aes(x=model,y=value,group=interaction(model,modscen),fill=variable),position = "dodge")+coord_flip()
