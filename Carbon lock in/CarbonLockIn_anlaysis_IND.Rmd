...
```{r}
# Relevant historical and bottom up data data
# historical IEA data (PE, SE, FE,Trade), and CEDS emissions data
setConfig(forcecache = TRUE)
historical <- calcOutput("Historical",aggregate = T)# aggregate = T not working at the moment, use when available
historical <- collapseNames(historical,collapsedim = 2)
hist_IEA_CEDS <- as.data.frame(historical) 
hist_IEA_CEDS <- hist_IEA_CEDS[,-1]
hist_IEA_CEDS <- hist_IEA_CEDS[,-3]
names(hist_IEA_CEDS)[3] <- "Variable"
...
hist_and_botmup <- rbind(hist_IEA_CEDS,hist_IRENA_cap,nuclear_IAEA,coal_EndCoal,capacity_costs_IEA) 
# historical nuclear 
...
ggplot()+
  ...
  geom_line(data = whole_a %>% filter(scen_name %in% c("NDC","NPi") ),aes(x=period,y=value/1000,group=interaction(model,scen_name),color=model,linetype=scen_name)) +
  labs(
    x = "Year" ,
...
ggplot()+
  ...
  ylim(0,1300)+ theme_bw()+ylab("Power Capacity (GW)")+coord_flip()
