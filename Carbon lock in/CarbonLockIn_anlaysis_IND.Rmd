...
line_type_1 <- function(data_s,hist_s,years){
  ...
  ggsave(file=paste(gsub("\\|","_ ",unique(data_s$variable)),"_1",".png",sep=""))
}

line_type_2 <- function(data_s,...){
  ggplot()+
...
geom_line(data = data_s %>% filter(!model %in% c("India MARKAL","AIM/Enduse 3.0")), aes(x=period,y=value,group=interaction(model,scen_name),color=model,linetype=scen_name))+
  ...
  annotate(geom = "point",x = c(2005,2010),y=hist_s$Value)
  ggsave(file=paste(gsub("\\|","_ ",unique(data_s$variable)),"_2",".png",sep=""),width=8,height=4)
  }
...
```{r,,message=FALSE,warning=FALSE,error=FALSE,purl=FALSE}
plotvar <- "Secondary Energy|Electricity"
whole_se_elec <- whole[whole$variable==plotvar,]
hist_se_elec <- hist_and_botmup %>% filter(Region=="IND", Year %in% c(2005,2010),Variable =="SE|Electricity (EJ/yr)") 

line_type_1(whole_se_elec,hist_se_elec,c(2005,2010))
line_type_2(whole_se_elec,hist_se_elec,c(2005,2010))

unique(hist_and_botmup$Variable[grep("SE|Electricity (EJ/yr)",hist_and_botmup$Variable)])
```
...
cap_cost <- whole[whole$variable==plotvar,]
