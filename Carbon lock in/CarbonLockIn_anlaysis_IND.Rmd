...
#line_type_1(whole_emi_elec_share,hist_emi_elec_share)
...
whole_se_elec_sum <- whole_se_elec %>% 
  group_by(scen_name,model,period,model_scope,unit,scenario) %>% 
...
ggplot()+
  geom_line(data = whole %>% filter(period<2050,variable=="Secondary Energy|Electricity|Coal") , aes(x=period,y=value,group=interaction(model,scen_name),linetype=model_scope,color=model))+facet_grid(~scen_name)+
 labs(
    x = "Year",
...
* There are some models which show a decrease in FE upon the start of climate mitigation. For e.g., AIM, IMAGE, WITCH.
...
hist_coal_cap <- hist_and_botmup%>% filter(Region=="IND", Variable =="Operating")
  ...
  line_type_2(whole_coal_cap,hist_coal_cap)
