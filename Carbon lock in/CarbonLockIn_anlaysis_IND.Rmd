...
geom_ribbon(data = whole_emi_elec_m %>% filter(model_scope=="national",period>=2015),aes(x = period,ymin = minimum,ymax=maximum),alpha = 0.1,fill="orange")+coord_cartesian(ylim=c(0,2500))+ labs(y="CO2 Emissions from Electricity  (MtCO2/yr)")+theme(axis.text=element_text(size=12), axis.title=element_text(size=13,face="bold"))
...
### DATA 
whole_emi_elec <- whole[whole$variable=="Emissions|CO2|Energy|Supply|Electricity",]
whole_cum <- whole_emi_elec %>%
  filter(period>=2010 & period<2035) %>%
...
# ggplot()+
#   geom_bar(data=whole_cum %>% filter(period==2050) , aes(x=model,y=cumsum, group=interaction(model,scen_name),fill=scen_name),position="dodge", stat = "identity")+
#   labs(
#     x = "Model name" ,
#     y = "Cumulative Emissions/Power sector (2011-2050) (GtCO2)",
#     fill = "Scenario Type",
#     alpha = "Model Scope"
#   ) +
#   # theme_bw()+
...
line_type_2(whole_se_elec_gas,hist_se_elec_gas)+ labs(
    x = "Year",
...
line_type_2(try_1,try_2)+theme(axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"))
...
line_type_2(whole_coal_cap_2,hist_coal_cap[hist_coal_cap$Variable=="Operating",])+
    geom_line(data = hist_2,aes(y = value/1000,x=Year),color="black",size=1.5)+
